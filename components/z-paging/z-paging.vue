<!--
 * @Author       : Zdd
 * @Date         : 2022-06-16 14:41:37
 * @LastEditors  : Zdd
 * @LastEditTime : 2022-06-30 19:47:05
 * @FilePath     : /components/z-paging/z-paging.vue
 * @Description  : 
...
<template name="z-paging">
  <!-- #ifndef APP-NVUE -->
  <view
    :class="!usePageScroll && fixed ? 'z-paging-content z-paging-content-fixed' : 'z-paging-content'"
  ...
  >
    ...
    <!-- #endif -->
    <!-- 顶部固定的slot -->
		<slot v-if="!usePageScroll && slotsShow('top')" name="top" />
		<view class="zp-page-top" v-else-if="usePageScroll && slotsShow('top')" :style="[{'top':`${windowTop}px`,'z-index':topZIndex}]">
			<slot name="top" />
		</view>
    <view :class="{ 'zp-view-super': true, 'zp-scroll-view-super': !usePageScroll }" :style="[finalScrollViewStyle]">
      <view v-if="slotsShow('left')" :class="{ 'zp-page-left': true, 'zp-absoulte': finalIsOldWebView }">
        <slot name="left" />
      </view>
      <view
        :class="{
          'zp-scroll-view-container': true,
          'zp-absoulte': finalIsOldWebView,
        }"
      ...
      >
        <scroll-view
          ...
          :class="{
            'zp-scroll-view': true,
            'zp-scroll-view-absolute': !usePageScroll,
            'zp-scroll-view-hide-scrollbar': !showScrollbar,
          }"
          :scroll-top="scrollTop"
          :scroll-x="scrollX"
          :scroll-y="
            scrollable && !usePageScroll && scrollEnable && (refresherCompleteScrollable ? true : refresherStatus !== 3)
          "
        ...
        >
          <view class="zp-paging-touch-view" 
            ...
            @touchcancel="_refresherTouchend"
            <!-- #endif -->
            <!-- #ifdef APP-VUE || MP-WEIXIN || MP-QQ || H5 -->
            @touchstart="pagingWxs.touchstart" @touchmove="pagingWxs.touchmove" @touchend="pagingWxs.touchend"
            @touchcancel="pagingWxs.touchend" @mousedown="pagingWxs.mousedown" @mousemove="pagingWxs.mousemove"
            @mouseup="pagingWxs.mouseup" @mouseleave="pagingWxs.mouseleave"
            <!-- #endif -->
            >
            <view
              ...
              :style="[
                {
                  background: refresherFixedBackground,
                  height: `${finalRefresherFixedBacHeight}px`,
                },
              ]"
            ></view>
            <view
              ...
              :style="[
                scrollViewInStyle,
                {
                  transform: finalRefresherTransform,
                  transition: refresherTransition,
                },
              ]"
              <!-- #ifdef APP-VUE || MP-WEIXIN || MP-QQ || H5 -->
              ...
              :data-pageScrollTop="wxsPageScrollTop" :data-scrollTop="wxsScrollTop"
              :data-refresherMaxAngle="refresherMaxAngle" :data-refresherAecc="refresherAngleEnableChangeContinued"
              :data-usePageScroll="usePageScroll" :data-watchTouchDirectionChange="watchTouchDirectionChange"
              :data-oldIsTouchmoving="isTouchmoving" :data-refresherOutRate="finalRefresherOutRate"
              :data-hasTouchmove="hasTouchmove"
              <!-- #endif -->
              <!-- #ifdef APP-VUE || H5 -->
              :change:renderPropScrollTop="pagingRenderjs.renderPropScrollTopChange"
              :renderPropScrollTop="renderPropScrollTop"
              :change:renderPropUsePageScroll="pagingRenderjs.renderPropUsePageScrollChange"
              :renderPropUsePageScroll="renderPropUsePageScroll"
              <!-- #endif -->
              >
              <view
                v-if="showRefresher"
                ...
                :style="[
                  {
                    'margin-top': `-${finalRefresherThreshold}px`,
              ...
              >
                <view
                  ...
                  :style="[
                    {
                      height: `${finalRefresherThreshold}px`,
                      background: refresherBackground,
                    },
                  ]"
                >
                  ...
                  <view class="zp-custom-refresher-slot-view">
                    <slot
                      v-if="!(slotsShow('refresherComplete') && refresherStatus === 3)"
                      :refresherStatus="refresherStatus"
                      name="refresher"
                    />
                  </view>
                  <slot v-if="slotsShow('refresherComplete') && refresherStatus === 3" name="refresherComplete" />
                  <paging-refresh
                    ref="refresh"
              ...
              <view class="zp-paging-container">
                <slot
                  v-if="useChatRecordMode && slotsShow('chatLoading') && loadingStatus !== 2 && realTotalData.length"
                  name="chatLoading"
                />
                <view
                  v-else-if="useChatRecordMode && loadingStatus !== 2 && realTotalData.length"
                  class="zp-chat-record-loading-container"
                >
                  <text
                    v-if="loadingStatus !== 1"
                    @click="_scrollToUpper()"
                    :class="
                      defaultThemeStyle === 'white'
                        ? 'zp-loading-more-text zp-loading-more-text-white'
                        : 'zp-loading-more-text zp-loading-more-text-black'
                    "
                    >{{ chatRecordLoadingMoreText }}</text
                  >
                  <image v-else :src="base64Flower" class="zp-chat-record-loading-custom-image" />
                </view>
                <!-- 全屏Loading -->
                <slot v-if="slotsShow('loading') && showLoading && !loadingFullFixed" name="loading" />
                <!-- 主体内容 -->
                <view
                  ...
                  :style="[
                    {
                      transform:
                        virtualPlaceholderTopHeight > 0 ? `translateY(${virtualPlaceholderTopHeight}px)` : 'none',
                    },
                ...
                >
                  ...
                  <slot
                    v-else-if="
                      loadingStatus === 1 && slotsShow('loadingMoreLoading') && showLoadingMore && loadingMoreEnabled
                    "
                  ...
                  <paging-load-more
                    ...
                    v-else-if="
                      ...
                      !(loadingStatus === 2 && !showLoadingMoreNoMoreView) &&
                      loadingMoreEnabled &&
                      !useChatRecordMode
                    "
                    :zConfig="zPagingLoadMoreConfig"
                  />
                  <view
                    v-if="safeAreaInsetBottom && useSafeAreaPlaceholder"
                ...
                <view
                  :class="{
                    'zp-empty-view': true,
                    'zp-empty-view-center': emptyViewCenter,
                  }"
                  :style="[{ emptyViewSuperStyle }]"
                  v-if="showEmpty"
                >
                  <slot v-if="slotsShow('empty')" name="empty" />
                  <z-paging-empty-view
                    v-else
      ...
      <view
        v-if="slotsShow('right')"
        :class="{
          'zp-page-right': true,
          'zp-absoulte zp-right': finalIsOldWebView,
        }"
      >
        <slot name="right" />
      </view>
    ...
    <!-- 底部固定的slot -->
    <slot v-if="!usePageScroll && slotsShow('bottom')" name="bottom" />
    <view class="zp-page-bottom" v-else-if="usePageScroll && slotsShow('bottom')" :style="[{ bottom: `${windowBottom}px` }]">
      <slot name="bottom" />
    </view>
    <!-- 点击返回顶部view -->
    <view
      v-if="showBackToTopClass"
      :class="backToTopClass"
      :style="[finalBackToTopStyle]"
      @click.stop="_backToTopClick"
    >
      <slot v-if="slotsShow('backToTop')" name="backToTop" />
      <image v-else class="zp-back-to-top-img" :src="backToTopImg.length ? backToTopImg : base64BackToTop" />
    </view>
    <!-- 全屏Loading(铺满z-paging并固定) -->
    <view v-if="slotsShow('loading') && showLoading && loadingFullFixed" class="zp-loading-fixed">
      <slot name="loading" />
    </view>
  </view>
  <!-- #endif -->
  <!-- #ifdef APP-NVUE -->
  <view
    :is="finalNvueSuperListIs"
    :style="[finalPagingStyle]"
    :class="{ 'z-paging-content-fixed': fixed && !usePageScroll }"
    :scrollable="false"
  >
    <!-- 顶部固定的slot -->
    <view
    ...
    <view :is="finalNvueSuperListIs" class="zp-n-list-container" :scrollable="false">
      <view
        ...
        :style="[
          {
            marginTop: scrollViewStyle.marginTop,
            marginBottom: scrollViewStyle.marginBottom,
          },
        ]"
      >
        <slot name="left" />
      ...
      <view
        ...
        :id="nvueListId"
        :style="[
          { flex: 1, top: isIos ? '0px' : '-1px' },
          scrollViewStyle,
          useChatRecordMode
            ? {
                transform: nIsFirstPageAndNoMore ? 'rotate(0deg)' : 'rotate(180deg)',
              }
            : {},
        ]"
        :is="finalNvueListIs"
        ...
        :show-scrollbar="showScrollbar && !useChatRecordMode"
        :loadmoreoffset="finalLowerThreshold"
        :enable-back-to-top="enableBackToTop"
        :scrollable="scrollable && scrollEnable && (refresherCompleteScrollable ? true : refresherStatus !== 3)"
        :bounce="nvueBounce"
      ...
      >
        <refresh
          ...
          v-if="finalNvueRefresherEnabled"
        ...
        <view
          ...
          :style="[
            {
              transform: `translateY(-${nShowRefresherRevealHeight}px)`,
              height: '0px',
            },
            { background: refresherBackground },
          ]"
        ...
        <template v-if="finalUseInnerList">
          ...
          <view
            class="zp-list-cell"
            :is="nViewIs"
            v-for="(item, index) in realTotalData"
        ...
        <view
          ...
          v-if="slotsShow('left')loading && showLoading && !loadingFullFixed"
          :is="nViewIs"
        >
          <slot name="loading" />
        </view>
        ...
        <view :is="nViewIs" v-if="!refresherOnly && loadingMoreEnabled">
          <view v-if="useChatRecordMode">
            <view v-if="loadingStatus !== 2 && realTotalData.length">
              <slot v-if="slotsShow('chatLoading')" name="chatLoading" />
          ...
          <view
            :style="
              nLoadingMoreFixedHeight
                ? {
                    height:
                      loadingMoreCustomStyle && loadingMoreCustomStyle.height ? loadingMoreCustomStyle.height : '80rpx',
                  }
                : {}
            "
          >
            ...
            <paging-load-more
              @doClick="_onLoadingMore('click')"
              v-else-if="showLoadingMoreCustom"
              :zConfig="zPagingLoadMoreConfig"
            />
            <view
              v-if="safeAreaInsetBottom && useSafeAreaPlaceholder"
      ...
      <view v-if="slotsShow('right')" class="zp-page-right">
        <slot name="right" />
      </view>
    </view>
    <!-- 底部固定的slot -->
