<template>
  <view class="u-calendar-month-wrapper" ref="u-calendar-month-wrapper">
    ...
    >
      ...
      >
      <view
        v-if="[monthIndex, monthIndex - 1, monthIndex + 1].includes(index)"
        class="u-calendar-month__days"
      >
        <view
          v-if="showMark"
...
export default {
  ...
  computed: {
    ...
    selectedChange() {
      return [this.minDate, this.maxDate, this.defaultDate, this.dateRange];
    },
  ...
  methods: {
    ...
    // 设置默认日期
    setDefaultDate() {
      if (!this.defaultDate && this.dateRange.length === 0) {
      ...
      if (this.mode === "single") {
        ...
        defaultDate =
          this.dateRange.length > 0 ? this.dateRange : this.defaultDate;
        // 如果为非数组，则不执行
        if (!uni.$u.test.array(defaultDate)) return;
      }
