  ...
  <view class="u-form-item">
    ...
    >
      ...
      <view class="u-form-item__body__right">
        <view
          class="u-form-item__body__right__content"
 ...
 * @property {String}						rightPosition	右侧区域的位置，top-上，center-中，bottom-下 ( 默认 'center' ）
 * @property {String}           labelStyle      lable的样式，对象形式
...
export default {
  ...
  computed: {
    lineStyle() {
      return `margin-top: ${
        this.message && this.parentData.errorType === "message" ? "5px" : 0
      }`;
    },
    lineColor() {
      return this.message && this.parentData.errorType === "border-bottom"
        ? uni.$u.color.error
        : uni.$u.props.line.color;
    },
    leftContentLabelStyle() {
      ...
      return [
        this.labelStyle || this.parentData.labelStyle,
        {
          justifyContent: justifyContent[this.parentData.labelAlign],
        },
      ];
    },
    rightContentStyle() {
      ...
      return {
        alignItems: positions[this.rightPosition],
      };
    },
    errorMessageStyle() {
      const labelWidth = uni.$u.addUnit(
        this.parentData.labelPosition === "top"
          ? 0
          : this.labelWidth || this.parentData.labelWidth,
      );
      return {
        marginLeft: `calc(8rpx + ${labelWidth})`,
      };
    },
  },
  mounted() {
